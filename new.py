...
async def city(message: Message, state: FSMContext):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
                f'https://api.openweathermap.org/data/2.5/weather?q={user_data["city"]}&appid={WEATHER_TOKEN}&units=metric&lang=ru') as response:
            if response.status == 200:
                ...
                weather_report = (
                    f'В городе {user_data["city"]} где живет {user_data["name"]} возраст {user_data["age"]} \n'
